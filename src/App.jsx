...
const UtilityFormView = ({ user, setView, showNotification, handleSaveUtilityLog, editData, setEditData, utilityLogs }) => {
  ...
  const [formData, setFormData] = useState(isEditing ? editData : { 
      ...
      elec1: { bt: '', cd: '', td: '', vc: '' },
      elec2: { bt: '', cd: '', td: '', vc: '' },
      water: { tong: '', tuoiCay: '', vanPhong: '', nhaAnVpc: '', nhaAnXuong: '', congChinh: '', congPhu: '', nvsXuong: '' }
  });
          ...
          <div className="space-y-4 animate-fade-in">
              ...
              <div className={`p-4 rounded-xl border flex items-center justify-between shadow-sm transition-colors ${isLowCosPhi ? 'bg-red-50 border-red-200' : 'bg-slate-50 border-slate-200'}`}>
                  <div className="flex items-center space-x-2">
                      ...
                      <span className={`font-bold ${isLowCosPhi ? 'text-red-700' : 'text-slate-700'}`}>Hệ số $\cos\varphi$:</span>
                  </div>
              ...
              {isLowCosPhi && (
                  <div className="flex items-start space-x-2 text-red-600 bg-red-100 p-3 rounded-lg text-sm font-medium">
                      ...
                      <p>Cảnh báo: $\cos\varphi$ dưới 0.9. Kiểm tra lại tụ bù hoặc liên hệ kỹ thuật!</p>
                  </div>
              )}
          </div>
      );
  ...
  const waterFields = [
      ...
      { id: 'congPhu', label: 'Bảo vệ cổng phụ' }, { id: 'nvsXuong', label: 'NVS Xưởng' }
  ];
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubMachines = onSnapshot(collection(db, 'artifacts', safeAppId, 'public', 'data', 'machines'), (snap) => setMachines(snap.docs.map(d => ({ ...d.data(), id: d.id }))), console.error);
    const unsubInventory = onSnapshot(collection(db, 'artifacts', safeAppId, 'public', 'data', 'inventory'), (snap) => setInventory(snap.docs.map(d => ({ ...d.data(), id: d.id }))), console.error);
    const unsubLogs = onSnapshot(collection(db, 'artifacts', safeAppId, 'public', 'data', 'logs'), (snap) => setLogs(snap.docs.map(d => ({ ...d.data(), id: d.id })).sort((a,b) => b.id - a.id)), console.error);
